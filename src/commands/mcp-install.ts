...
export function cliInvocation(): CliInvocation {
  // The mcp surface is no longer alpha, so the config we write must NOT carry
  // --accept-alpha — the relay now rejects that flag. (Configs written by an older
  // CLI still carry it and will fail loudly, prompting a re-install.)
